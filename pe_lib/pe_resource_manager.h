...
public:
	//ID; string
	typedef std::map<WORD, std::wstring> string_list;
	...
	typedef std::map<DWORD, message_table_item> message_list;
...
public:
	...
	public:
		...
		resource_data_info(const std::string& data, DWORD codepage);
		//Constructor from data
...
public:
	...
	typedef std::vector<DWORD> resource_type_list;
	typedef std::vector<DWORD> resource_id_list;
	typedef std::vector<std::wstring> resource_name_list;
	typedef std::vector<DWORD> resource_language_list;
	...
	typedef std::multimap<WORD, WORD> translation_values_map;
...
public:
	...
	unsigned long get_language_count(resource_type type, DWORD id) const;
	//Returns language count of resource by resource name and ID
	unsigned long get_language_count(const std::wstring& root_name, DWORD id) const;
	...
	const resource_language_list list_resource_languages(resource_type type, DWORD id) const;
	//Lists resource languages by resource name and ID
	const resource_language_list list_resource_languages(const std::wstring& root_name, DWORD id) const;

	//Returns raw resource data by type, name and language
	const resource_data_info get_resource_data_by_name(DWORD language, resource_type type, const std::wstring& name) const;
	//Returns raw resource data by root name, name and language
	const resource_data_info get_resource_data_by_name(DWORD language, const std::wstring& root_name, const std::wstring& name) const;
	//Returns raw resource data by type, ID and language
	const resource_data_info get_resource_data_by_id(DWORD language, resource_type type, DWORD id) const;
	//Returns raw resource data by root name, ID and language
	const resource_data_info get_resource_data_by_id(DWORD language, const std::wstring& root_name, DWORD id) const;
	//Returns raw resource data by type, name and index in language directory (instead of language)
	const resource_data_info get_resource_data_by_name(resource_type type, const std::wstring& name, DWORD index = 0) const;
	//Returns raw resource data by root name, name and index in language directory (instead of language)
	const resource_data_info get_resource_data_by_name(const std::wstring& root_name, const std::wstring& name, DWORD index = 0) const;
	//Returns raw resource data by type, ID and index in language directory (instead of language)
	const resource_data_info get_resource_data_by_id(resource_type type, DWORD id, DWORD index = 0) const;
	//Returns raw resource data by root name, ID and index in language directory (instead of language)
	const resource_data_info get_resource_data_by_id(const std::wstring& root_name, DWORD id, DWORD index = 0) const;

	//Returns bitmap data by name and language (minimum checks of format correctness)
	const std::string get_bitmap_by_name(DWORD language, const std::wstring& name) const;
	//Returns bitmap data by name and index in language directory (instead of language) (minimum checks of format correctness)
	const std::string get_bitmap_by_name(const std::wstring& name, DWORD index = 0) const;
	//Returns bitmap data by ID and language (minimum checks of format correctness)
	const std::string get_bitmap_by_id_lang(DWORD language, DWORD id) const;
	//Returns bitmap data by ID and index in language directory (instead of language) (minimum checks of format correctness)
	const std::string get_bitmap_by_id(DWORD id, DWORD index = 0) const;

	//Returns icon data by name and language (minimum checks of format correctness)
	const std::string get_icon_by_name(DWORD language, const std::wstring& icon_group_name) const;
	//Returns icon data by name and index in language directory (instead of language) (minimum checks of format correctness)
	const std::string get_icon_by_name(const std::wstring& icon_group_name, DWORD index = 0) const;
	//Returns icon data by ID and language (minimum checks of format correctness)
	const std::string get_icon_by_id_lang(DWORD language, DWORD icon_group_id) const;
	//Returns icon data by ID and index in language directory (instead of language) (minimum checks of format correctness)
	const std::string get_icon_by_id(DWORD icon_group_id, DWORD index = 0) const;

	//Returns cursor data by name and language (minimum checks of format correctness)
	const std::string get_cursor_by_name(DWORD language, const std::wstring& cursor_group_name) const;
	//Returns cursor data by name and index in language directory (instead of language) (minimum checks of format correctness)
	const std::string get_cursor_by_name(const std::wstring& cursor_group_name, DWORD index = 0) const;
	//Returns cursor data by ID and language (minimum checks of format correctness)
	const std::string get_cursor_by_id_lang(DWORD language, DWORD cursor_group_id) const;
	//Returns cursor data by ID and index in language directory (instead of language) (minimum checks of format correctness)
	const std::string get_cursor_by_id(DWORD cursor_group_id, DWORD index = 0) const;

	//Returns string table data by ID and language
	const string_list get_string_table_by_id_lang(DWORD language, DWORD id) const;
	//Returns string table data by ID and index in language directory (instead of language)
	const string_list get_string_table_by_id(DWORD id, DWORD index = 0) const;
	//Returns string from string table by ID and language
	const std::wstring get_string_by_id_lang(DWORD language, WORD id) const;
	//Returns string from string table by ID and index in language directory (instead of language)
	const std::wstring get_string_by_id(WORD id, DWORD index = 0) const;

	//Returns message table data by ID and language
	const message_list get_message_table_by_id_lang(DWORD language, DWORD id) const;
	//Returns message table data by ID and index in language directory (instead of language)
	const message_list get_message_table_by_id(DWORD id, DWORD index = 0) const;
...
public: //VERSION INFO
	...
	public: //Setters
		...
		void set_file_flags(DWORD file_flags);

		//Sets file version most significant DWORD
		void set_file_version_ms(DWORD file_version_ms);
		//Sets file version least significant DWORD
		void set_file_version_ls(DWORD file_version_ls);
		//Sets product version most significant DWORD
		void set_product_version_ms(DWORD product_version_ms);
		//Sets product version least significant DWORD
		void set_product_version_ls(DWORD product_version_ls);

		//Sets file OS type (raw DWORD)
		void set_file_os_raw(DWORD file_os);
		//Sets file OS type
		void set_file_os(file_os_type file_os);

		//Sets file type (raw DWORD)
		void set_file_type_raw(DWORD file_type);
		//Sets file type
		void set_file_type(file_type file_type);

		//Sets file subtype (usually non-zero for drivers and fonts)
		void set_file_subtype(DWORD file_subtype);

		//Sets file date most significant DWORD
		void set_file_date_ms(DWORD file_date_ms);
		//Sets file date least significant DWORD
		void set_file_date_ls(DWORD file_date_ls);

	private:
		//Helper to convert version DWORDs to string
		template<typename T>
		static const std::basic_string<T> get_version_string(DWORD ms, DWORD ls)
		{
		...
		void set_file_flag(DWORD flag);
		//Helper to clear file flag
		void clear_file_flag(DWORD flag);
		//Helper to set or clear file flag
		void set_file_flag(DWORD flag, bool set_flag);

		DWORD file_version_ms_, file_version_ls_,
			product_version_ms_, product_version_ls_;
		DWORD file_flags_;
		DWORD file_os_;
		DWORD file_type_, file_subtype_;
		DWORD file_date_ms_, file_date_ls_;
	};
	...
	const file_version_info get_version_info(lang_string_values_map& string_values, translation_values_map& translations, DWORD index = 0) const;
	const file_version_info get_version_info_by_lang(lang_string_values_map& string_values, translation_values_map& translations, DWORD language) const;
...
protected:
	...
	//Returns icon count
	WORD format_icon_headers(std::string& ico_data, const std::string& resource_data) const;
	//Helper function of creating cursor headers from CURSOR_GROUP resource data
	//Returns cursor count
	WORD format_cursor_headers(std::string& cur_data, const std::string& resource_data, DWORD language, DWORD index = 0xFFFFFFFF) const;
	//Helper function of parsing string list table
	const string_list parse_string_list(DWORD id, const std::string& resource_data) const;
...
protected: //VERSION INFO helpers
	...
	static const std::wstring version_info_key;

	//Returns aligned version block value position
	static DWORD get_version_block_value_pos(DWORD base_pos, const wchar_t* key);

	//Returns aligned version block first child position
	static DWORD get_version_block_first_child_pos(DWORD base_pos, DWORD value_length, const wchar_t* key);
...
public: //Resource editing
	...
	//Returns true if resource was deleted
	bool remove_resource(resource_type type, DWORD id);
	bool remove_resource(const std::wstring& root_name, DWORD id);

	//Removes resource language by resource type/root name and name
	//Deletes only one entry of given type, name and language
	//Returns true if resource was deleted
	bool remove_resource(resource_type type, const std::wstring& name, DWORD language);
	bool remove_resource(const std::wstring& root_name, const std::wstring& name, DWORD language);
	//Removes recource language by resource type/root name and ID
	//Deletes only one entry of given type, ID and language
	//Returns true if resource was deleted
	bool remove_resource(resource_type type, DWORD id, DWORD language);
	bool remove_resource(const std::wstring& root_name, DWORD id, DWORD language);
	
	//Adds resource. If resource already exists, replaces it
	//timestamp will be used for directories that will be added
	void add_resource(const std::string& data, resource_type type, const std::wstring& name, DWORD language, DWORD codepage = 0, DWORD timestamp = 0);
	void add_resource(const std::string& data, const std::wstring& root_name, const std::wstring& name, DWORD language, DWORD codepage = 0, DWORD timestamp = 0);
	//Adds resource. If resource already exists, replaces it
	//timestamp will be used for directories that will be added
	void add_resource(const std::string& data, resource_type type, DWORD id, DWORD language, DWORD codepage = 0, DWORD timestamp = 0);
	void add_resource(const std::string& data, const std::wstring& root_name, DWORD id, DWORD language, DWORD codepage = 0, DWORD timestamp = 0);

	//Adds bitmap from bitmap file data. If bitmap already exists, replaces it
	//timestamp will be used for directories that will be added
	void add_bitmap(const std::string& bitmap_file, DWORD id, DWORD language, DWORD codepage = 0, DWORD timestamp = 0);
	void add_bitmap(const std::string& bitmap_file, const std::wstring& name, DWORD language, DWORD codepage = 0, DWORD timestamp = 0);

	//Removes icon group and all its icons by name/ID and language
	void remove_icon_group(const std::wstring& icon_group_name, DWORD language);
	void remove_icon_group(DWORD icon_group_id, DWORD language);

	//Removes cursor group and all its cursors by name/ID and language
	void remove_cursor_group(const std::wstring& cursor_group_name, DWORD language);
	void remove_cursor_group(DWORD cursor_group_id, DWORD language);

	//Removes bitmap by name/ID and language
	void remove_bitmap(const std::wstring& name, DWORD language);
	void remove_bitmap(DWORD id, DWORD language);
	...
	void add_icon(const std::string& icon_file, const std::wstring& icon_group_name, DWORD language, icon_place_mode mode = icon_place_after_max_icon_id, DWORD codepage = 0, DWORD timestamp = 0);
	void add_icon(const std::string& icon_file, DWORD icon_group_id, DWORD language, icon_place_mode mode = icon_place_after_max_icon_id, DWORD codepage = 0, DWORD timestamp = 0);
	...
	void add_cursor(const std::string& cursor_file, const std::wstring& cursor_group_name, DWORD language, icon_place_mode mode = icon_place_after_max_icon_id, DWORD codepage = 0, DWORD timestamp = 0);
	void add_cursor(const std::string& cursor_file, DWORD cursor_group_id, DWORD language, icon_place_mode mode = icon_place_after_max_icon_id, DWORD codepage = 0, DWORD timestamp = 0);

	//Sets/replaces full version information:
	//file_version_info: versions and file info
	//lang_string_values_map: map of version info strings with encodings
	//translation_values_map: map of translations
	void set_version_info(const file_version_info& file_info, const lang_string_values_map& string_values, const translation_values_map& translations, DWORD language, DWORD codepage = 0, DWORD timestamp = 0);
...
private:
	...
	//Helper to remove resource
	bool remove_resource(const pe_base::resource_directory::entry_finder& root_finder, const pe_base::resource_directory::entry_finder& finder, DWORD language);

	//Helper to add/replace resource
	void add_resource(const std::string& data, resource_type type, pe_base::resource_directory_entry& new_entry, const pe_base::resource_directory::entry_finder& finder, DWORD language, DWORD codepage, DWORD timestamp);
	void add_resource(const std::string& data, const std::wstring& root_name, pe_base::resource_directory_entry& new_entry, const pe_base::resource_directory::entry_finder& finder, DWORD language, DWORD codepage, DWORD timestamp);
	void add_resource(const std::string& data, pe_base::resource_directory_entry& new_root_entry, const pe_base::resource_directory::entry_finder& root_finder, pe_base::resource_directory_entry& new_entry, const pe_base::resource_directory::entry_finder& finder, DWORD language, DWORD codepage, DWORD timestamp);

	//Add icon helper
	void add_icon(const std::string& icon_file, const resource_data_info* group_icon_info /* or zero */, pe_base::resource_directory_entry& new_icon_group_entry, const pe_base::resource_directory::entry_finder& finder, DWORD language, icon_place_mode mode, DWORD codepage, DWORD timestamp);
	
	//Add cursor helper
	void add_cursor(const std::string& cursor_file, const resource_data_info* group_cursor_info /* or zero */, pe_base::resource_directory_entry& new_cursor_group_entry, const pe_base::resource_directory::entry_finder& finder, DWORD language, icon_place_mode mode, DWORD codepage, DWORD timestamp);

	//Remove icon group helper
	void remove_icons_from_icon_group(const std::string& icon_group_data, DWORD language);

	//Remove cursor group helper
	void remove_cursors_from_cursor_group(const std::string& cursor_group_data, DWORD language);

	//Returns free icon or cursor ID list depending on icon_place_mode
	const std::vector<WORD> get_icon_or_cursor_free_id_list(resource_type type, icon_place_mode mode, DWORD count);
};
...
public:
	//Useful typedefs
	typedef std::pair<WORD, WORD> translation_pair;
...
public:
	...
	void add_translation(WORD language_id, WORD codepage_id);
	
	//Removes translation from translations and strings lists
	void remove_translation(const std::wstring& translation);
	void remove_translation(WORD language_id, WORD codepage_id);
